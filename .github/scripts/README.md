...
Stdlib `unittest` tests for the top-level scripts. `test_eject.py` covers the `eject.py` manifest loading and keep-set guard; `test_repository_setup.py` covers the `setup/repository-setup.py` engine: template copy, dry-run, the `--remove-path` migration, and that setup writes nothing outside the repository. Run from the repo root: `python .github/scripts/tests/test_eject.py` and `python .github/scripts/tests/test_repository_setup.py`. The learning pipeline keeps its own fixtures under `learning/tests/`.
